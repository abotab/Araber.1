...
def main_menu():
    text = f"**- [سـورس العرب](https://t.me/Haider_1h1)\
        \n المسـاعد\
        \n\
        \n◽ العرب لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس العرب 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس العرب 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس العرب 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت العرب 🧸♥](https://t.me/Jepthon)**\
            \n\
            \n❤ العرب هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/G_lRR)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
